    ...
    if (*numContas == 0) {
        ...
        for (int i = 0; i < *numContas; i++) {
            if ((*cs)[i].id != 0) {
                ...
                // Exibição da senha mascarada
                printf("* Senha do Usuario: ");
                for (int j = 0; j < strlen((*cs)[i].senha); j++) {
                    printf("*");
                }
                printf("\n");

                // Exibir o saldo corretamente
...
void CarregarContas(Conta_Steam **cs, int *numContas) {
    ...
    while (fscanf(arquivo, "ID: %d\n", &novaConta.id) != EOF) {
        fgets(buffer, sizeof(buffer), arquivo); // Lê "Nome: "
        sscanf(buffer, "Nome: %[^\n]", novaConta.nome);

        fgets(buffer, sizeof(buffer), arquivo); // Lê "Apelido: "
        sscanf(buffer, "Apelido: %[^\n]", novaConta.apelido);

        fgets(buffer, sizeof(buffer), arquivo); // Lê "Email: "
        sscanf(buffer, "Email: %[^\n]", novaConta.email);

        fgets(buffer, sizeof(buffer), arquivo); // Lê "Senha: "
        sscanf(buffer, "Senha: %[^\n]", novaConta.senha);

        fgets(buffer, sizeof(buffer), arquivo); // Lê "Saldo Total: "
        sscanf(buffer, "Saldo Total: R$%f", &novaConta.saldototal);

        fgets(buffer, sizeof(buffer), arquivo); // Lê linha de separação
